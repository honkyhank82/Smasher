  ...
  private handleAppError(error: AppError, context: ErrorContext): void {
    ...
    if (error.isOperational) {
      ...
      this.showUserFriendlyError(
        new AppError(ErrorMessages[ErrorCode.UNKNOWN_ERROR], {
          isOperational: false,
        }),
      );
    }
    ...
    if (response) {
      ...
      switch (status) {
        case 400:
          ...
          errorMessage =
            data?.message || ErrorMessages[ErrorCode.VALIDATION_ERROR];
          break;
        case 401:
        ...
        default:
          ...
          errorMessage =
            data?.message || ErrorMessages[ErrorCode.UNKNOWN_ERROR];
      }
    } else if (request) {
